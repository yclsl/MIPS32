...
def split_instruction(instruction, len_arr):
    if not type(instruction) is str:
...
def ins2str_impl(instructions, pos_ins_pair, init_pos, cache):
    ...
    if (is_category1_add(instruction)):
        return ins2str_category1_add(instruction, instruction_pos, cache, execute)
    elif (is_category1_J(instruction)):
        return ins2str_category1_J(instruction, instruction_pos, cache, execute)
    elif (is_category1_JR(instruction)):
        return ins2str_category1_JR(instruction, instruction_pos, cache, execute)
    elif (is_category1_BEQ(instruction)):
        return ins2str_category1_BEQ(instruction, instruction_pos, cache, execute)
    elif (is_category1_SLL(instruction)):
        return ins2str_category1_SLL(instruction, instruction_pos, cache, execute)
    elif (is_category1_LW(instruction)):
        return ins2str_category1_LW(instruction, instruction_pos, cache, execute)
    elif (is_category1_mult(instruction)):
        return ins2str_category1_mul(instruction, instruction_pos, cache, execute)
    elif (is_category1_BGTZ(instruction)):
        return ins2str_category1_BGTZ(instruction, instruction_pos, cache, execute)
    elif (is_category1_sub(instruction)):
        return ins2str_category1_sub(instruction, instruction_pos, cache, execute)
    elif (is_category1_SW(instruction)):
        return ins2str_category1_SW(instruction, instruction_pos, cache, execute)
    elif (is_category1_BLTZ(instruction)):
        return ins2str_category1_BLTZ(instruction, instruction_pos, cache, execute)
    elif (is_break(instruction)):
        return ins2str_break(instruction, instruction_pos, cache, execute)
    elif (is_category1_SRA(instruction)):
        return ins2str_category1_SRA(instruction, instruction_pos, cache, execute)
    elif (is_category1_SRL(instruction)):
        return ins2str_category1_SRL(instruction, instruction_pos, cache, execute)
    elif (is_category2_add(instruction)):
        return ins2str_category2_add(instruction, instruction_pos, cache, execute)
    elif (is_category2_sub(instruction)):
        return ins2str_category2_sub(instruction, instruction_pos, cache, execute)
    elif (is_category2_mul(instruction)):
        return ins2str_category2_mul(instruction, instruction_pos, cache, execute)
    elif (is_category2_and(instruction)):
        return ins2str_category2_and(instruction, instruction_pos, cache, execute)
    elif (is_category2_nor(instruction)):
        return ins2str_category2_nor(instruction, instruction_pos, cache, execute)
    elif (is_category2_SLT(instruction)):
        return ins2str_category2_SLT(instruction, instruction_pos, cache, execute)
    else:
        assert False, "没有匹配指令"
...
def execute_instruction_which_can_be_executed(instructions, init_pos, cache, content, circle):

    if (len(cache['Post-ALU Buffer']) > 0 and old_instruction(cache['Post-ALU Buffer'][0])):
        instruction_pos = get_instruction_pos(cache['Post-ALU Buffer'][0])
        execute_one_instruction(instructions, instruction_pos, init_pos, cache, content, True, circle)
        cache['Post-ALU Buffer'] = []

    if (len(cache['Post-ALUB Buffer']) > 0 and old_instruction(cache['Post-ALUB Buffer'][0])):
...
def is_if_unit(instruction):
    return is_category1_J(instruction) or is_category1_JR(instruction) or is_category1_BEQ(instruction) or is_category1_BLTZ(instruction) \
           or is_nop(instruction) or is_break(instruction) or is_category1_BGTZ(instruction)


def read_intructions_to_pre_issue_buffer(instructions, instruction_pos, init_pos, cache):
    ...
    for i in range(2):
        ...
        if(is_if_unit(instruction)):
            if(is_nop(instruction) or is_break(instruction) or is_category1_J(instruction)):
                cache['IF Unit:Executed Instruction'].append([instruction_pos, instruction, True, 0])
            else:
                cache['IF Unit:Waiting Instruction'] = [[instruction_pos, instruction, True, 0]]
            cache['stop_read_instruction'] = True
            return instruction_pos
        else:
            cache['Pre-Issue Buffer'].append([instruction_pos, instruction, True, 0])
...
def other_bq(instruction, cache):
    ...
    all_q = [cache['Pre-ALU Queue'], cache['Post-ALU Buffer'], cache['Pre-ALUB Queue'], cache['Post-ALUB Buffer'], cache['Pre-MEM Queue'], cache['Post-MEM Buffer']]
    qq = []
    for t in all_q:
        if not t is q:
            qq.extend(t)
...
def add_instruction2queue(add_queue_element, cache):
    ...
    for index in add_queue_element:
        ...
        if (len(q) < 2):
            ...
            cache['Pre-Issue Buffer'] = remove_index_of_lst(cache['Pre-Issue Buffer'], index-cnt)
            cnt = cnt + 1
...
def if_unit2buffer(instructions, init_pos, cache, content, circle):
    if (len(cache['IF Unit:Waiting Instruction']) == 1):
        if (if_wait_unit_can_execute(cache) and old_instruction(cache['IF Unit:Waiting Instruction'][0])):
...
def execute_if_unit(instructions, init_pos, cache, content, circle):
            #有没有这条指令   并呆过一段时间
    if (len(cache['IF Unit:Executed Instruction']) > 0 and old_instruction(cache['IF Unit:Executed Instruction'][0])):
        instruction_pos = get_instruction_pos(cache['IF Unit:Executed Instruction'][0])
        next_pos = execute_one_instruction(instructions, instruction_pos, init_pos, cache, content, True, circle)
        cache['IF Unit:Executed Instruction'] = []
        cache['stop_read_instruction'] = False
        return next_pos
...
def execute_instructions(instructions, init_pos, cache, content, execute):
    ...
    while (True):
        ...
        if (not cache['stop_read_instruction']):
            read_pos = read_intructions_to_pre_issue_buffer(instructions, read_pos, init_pos, cache)
        ...
        # 返回下一个地址
        next_pos = execute_instruction_which_can_be_executed(instructions, init_pos, cache, content, cycle)
        queue2buffer(instructions, init_pos, cache, [], cycle)
        set_all_ins_old(cache)
        #设置为旧的，证明执行过。所以要记录它执行过几个轮次
        add_one_count2alub(cache)
        ret.append("--------------------\nCycle:" + str(cycle) + "\n\n" + cache_state(instructions, init_pos, cache))
        if(finished(cache)):
            break
        cycle += 1
    return ret
